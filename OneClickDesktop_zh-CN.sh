...
GUACAMOLE_DOWNLOAD_LINK="http://d.junlin.li/vps/guacamole-server-1.5.5.tar.gz"
...
function get_user_options
{
	...
	if [ "x$OS" != "xCENTOS8" ] && [ "x$OS" != "xCENTOS7" ] ; then
		say @B"本脚本仅支持Guacamole通过RDP连接Linux桌面，请输入1按回车" yellow
		read choice_rdpvnc
	else 
	...
	if [ $choice_rdpvnc = 1 ] ; then
		...
		echo "默认分辨率1280x1024请输入1, 自适应分辨率请输入2, 手动设置分辨率请输入3."
...
function install_guacamole_ubuntu_debian
{
	...
	say @B"安装依赖环境..." yellow
	echo 
	apt-get update && apt-get upgrade -y && apt install openjdk-17-jdk -y
	wget http://d.junlin.li/vps/apache-tomcat-9.0.91.tar.gz && tar xzf apache-tomcat-9.0.91.tar.gz && mv apache-tomcat-9.0.91 /var/lib/tomcat9
	echo "export CATALINA_HOME="/var/lib/tomcat9"" >> ~/.bashrc
	source ~/.bashrc
	useradd -r tomcat
	chown -R tomcat:tomcat /var/lib/tomcat9
	wget -P /lib/systemd/system http://d.junlin.li/vps/tomcat9.service
...
function main
{
	...
	if [ "$OS" = "CENTOS7" ] || [ "$OS" = "CENTOS8" ] ; then
		...
		say @B"此桌面环境需要至少1 GB内存。此脚本仅支持 Debian 12。" yellow
	fi
	echo 
	echo "请问是否继续？ [Y/N]"
	...
	echo 
	echo "运行命令 dpkg-reconfigure locales 可将服务器语言设置成中文，然后重启服务器。"
	echo "安装完成后建议重启一次服务器。"
}
